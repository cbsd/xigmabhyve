...
$gt_selection_destroy_confirm = gtext('Do you want to destroy the selected VM(s)?');
$img_path = [
	'add' => 'images/add.png',
	'mod' => 'images/edit.png',
...
if($_POST):
	...
	if(isset($_POST['destroy_selected_jail']) && $_POST['destroy_selected_jail']):
		...
		foreach($checkbox_member_array as $checkbox_member_record):
			if(false !== ($index = array_search_ex($checkbox_member_record, $sphere_array, 'jailname'))):
				if(!isset($sphere_array[$index]['protected'])):
					$cmd = ("/usr/local/bin/cbsd bdestroy {$checkbox_member_record}");
					$return_val = mwexec($cmd);
					if($return_val == 0):
						//$savemsg .= gtext("Jail(s) restarted successfully.");
						header($sphere_header);
...
<?php
					...
					$vnc_len=strlen($sphere_record['vnc']);
					$ssh1_len--;
					$ssh2_len--;
					$vnc_len--;
?>
